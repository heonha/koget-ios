...
struct WidgetGridCell: View {
    ...
    var cellWidth: CGFloat = Constants.deviceSize.width / 4.3
    ...
    init(name: String, url: String, widgetImage: UIImage, viewModel: MainWidgetViewModel) {
        self.name = name
        self.url = url
    ...
    var body: some View {
        VStack(spacing: 2) {
            ...
            ZStack {
                ...
                Image
                    .uiImage(widgetImage.addClearBackground(sourceResizeRatio: 1.0) ?? CommonImages.emptyIcon)
                    .resizable()
                    .scaledToFill()
                    .clipShape(Circle())
            }
...
struct WidgetButton_Previews: PreviewProvider {
    static var previews: some View {
        WidgetGridCell(name: "위젯이름", url: "https://google.com", widgetImage: UIImage.init(named: "navermap")!, viewModel: MainWidgetViewModel())
    }
}
